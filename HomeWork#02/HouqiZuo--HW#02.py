...
search=set(query)
# traversal for find "and"
# if exist, set boolOperator = 1
boolOperator = 1
for tempWord in search:
	if( tempWord == "or" ):
		boolOperator = 0
	if ( tempWord == "and"):
		boolOperator = 1
		break
# delete the "and " and "or"
search = list(search)
for temp in search:
	if( temp == "and"):
		search.remove("and")
...
if (boolOperator):
	...
	for i,quote in enumerate(data_list):
		words = quote.split()
		# result = [ 0 for i in range( len(search) )]
		result = [0] * len(search) # store the how many words they have
		# traversal for each elements of words collection
		for word in words:
			# traversal for each elements of search collection
			for m, temp_search in enumerate(search):
				# compare the each elements
				if( word == temp_search):
					result[m] = 1
		# output
		temp_state = True
